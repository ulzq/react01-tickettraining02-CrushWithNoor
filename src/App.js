...
class App extends React.Component {
  ...
  */
  ...
  fire = async () => {
    ...
    await new Promise(resolve => {
      let timer = setInterval(() => {
        time += 0.033;
        cannonball.x =
          startX + player.power * time * Math.cos(player.angle * RAD);
        cannonball.y =
          startY +
          player.power * time * Math.sin(player.angle * RAD) -
          (GRAV * Math.pow(time, 2)) / 2;
  ...
  render() {
    ...
    let others = this.state.player.filter(player => {
      return player !== current;
    });
    let playerNames = this.state.player.map(player => {
      return player.name;
    });
    return (
      <div className="App">
        ...
        {this.state.showSettings ? (
          <div className="Settings">
            ...
            />
            <button
              className="start center-relative-h"
